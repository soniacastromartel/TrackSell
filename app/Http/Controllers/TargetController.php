...
class TargetController extends Controller
{
    ...
    public function index() {
        ...
        return view('calculate_incentives', ['title'      => $title
                                             ,'centres'   => $centres
                                             ,'employees' => $employees
    ...
    public function importIncentive(Request $request) {
        ...
        try {
            ...
            if($request->hasFile('targetInputIncentiveFile')) {
                ...
                return redirect('/admin/incentives')->with(['title' => 'Calculadora de incentivos'
                ...
                if (!empty($target)){
                    ...
                    foreach($centres as $centre) {  
                        ...
                        if (isset($target[$centre->id]) && !empty($target[$centre->id])) {
                            $targetData[$centre->name][$currentYear][$month] = ['obj_vc'        => $target[$centre->id]->obj1,
                        ...
                        if (!isset($totalTargetByIsland[$centre->island][$currentYear][$month])) {
                            ...
                            $totalTargetByIsland[$centre->island][$currentYear][$month] = ['obj_vc'             => 0
                                                                                        ,'obj_vp'            => 0
                                                                                        ,'vc'                => 0
                                                                                        ,'vp'                => 0 
                                                                                        ,'cont_employees'    => 0 
                    ...
                    foreach ( $totalTargetByIsland[$island] as $year => $totalTargetMonthYear) {
                        foreach ($totalTargetMonthYear as $month => $totalTarget){
                            ...
                            if (!isset($totalTargetICOT[$year][$month])){
                                $totalTargetICOT[$year][$month] = ['obj_vc'              => 0
                                                                ,'obj_vp'            => 0
                                                                ,'vc'                => 0
                                                                ,'vp'                => 0 
                                                                ,'cont_employees'    => 0 
