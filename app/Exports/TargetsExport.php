...
class TargetsExport implements
    FromCollection,
    WithStyles,
    WithEvents
{
    ...
    private function printContentSecondSheet($target, $centres, $sheet)
    {
        ...
        foreach ($centres as $centre) {
            ...
            foreach ($totalCentre as $tc => $totalDetail) {
                ...
                $sheet->setCellValue('B' . $this->rows, ' ');
                $sheet->setCellValue('D' . $this->rows, $totalDetail['total_incentive']);
                $sheet->setCellValue('E' . $this->rows, $totalDetail['total_super_incentive']);
                $sheet->setCellValue('F' . $this->rows, $totalDetail['total_income']);
                $sheet->getStyle('A' . $this->rows . ':F' . $this->rows)->getFont()->setBold(true);
                $sheet->getStyle('A' . $this->rows . ':F' . $this->rows)->applyFromArray(
                    $style1
                );
                $sheet->getStyle('A' . $this->rows . ':F' . $this->rows)->getFill()->getStartColor()->setARGB('FFFFE0');
                $this->rows++;
