...
class Employee extends Authenticatable
{
    ...
    protected $fillable = [
        'name', 'username', 'username_temp'
        , 'cancellation_date', 'password'
        , 'centre_id' , 'rol_id'
        , 'validated', 'pending_password'
        , 'username_temp'
        , 'dni' , 'phone' , 'mobile_phone'
        , 'email', 'category'
    ...
    public function scopeGetPrescriptorCenter(){
        ...
        $centres[] = [
            'centre'             => $principalCentre->label
            , 'centre_id'        => $principalCentre->id
            , 'centre_address'   => $principalCentre->address
            , 'centre_phone'     => $principalCentre->phone
            , 'centre_email'     => $principalCentre->email
            , 'timetable'        => $principalCentre->timetable 
            , 'island'           => $principalCentre->island
            , 'image'            => env('BASE_API_URL') .$principalCentre->image
            , 'principal'        => true
        ];
        ...
        if (!empty($principalCentre) && $principalCentre->island == 'TENERIFE') {
            ...
            foreach($tfeCentres as $c) {
                if ($c->id != $principalCentre->id) {
                    $centres[] = ['centre'           => $c->label
                                , 'centre_id'        => $c->id
                                , 'centre_address'   => $c->address
                                , 'centre_phone'     => $c->phone
                                , 'centre_email'     => $c->email
                                , 'timetable'        => $c->timetable 
                                , 'island'           => $c->island
                                , 'image'            => env('BASE_API_URL') .$c->image
                                , 'principal'        => false
                    ];
                }
            }
