  ...
  <div class="container-fluid" style="margin-top: 120px;">
    <div class="card">
        ...
        <div class="">
            ...
            <div class="col-md-4 centre-container">
                <h3>Centro</h3>
                <form action="{{ route('calculateServices') }}" method="GET">
                    <input type="hidden" name="service_id" value="{{ $service_id }}">
                    <select class="selectpicker" data-style="btn btn-red-icot btn-round" id="centre_id" name="centre_id" onchange="this.form.submit()">
                        <option value="">TODOS</option>
                        @foreach ($centres as $centre)
                            <option value="{{ $centre->id }}" {{ $centre_id == $centre->id ? 'selected' : '' }}>{{ $centre->name }}</option>
                        @endforeach
                    </select>
                </form>
            </div>
            <div class="col-md-4 service-container">
                ...
                <form id="centreForm" action="{{ route('calculateServices') }}" method="GET">
                    <input type="hidden" name="centre_id" value="{{ $centre_id }}">
                    <select class="selectpicker" data-style="btn btn-red-icot btn-round" id="service_id" name="service_id" onchange="this.form.submit()">
                        <option value="">TODOS</option>
        ...
        <div class="buttons-container" style="display:flex;justify-content:flex-end;">
            <button id="btnClear" class="btn-refresh" onclick="resetSelectors()">Limpiar Formulario <span id="icon-refresh" class="material-icons">refresh</span></button>
            <form action="{{ route('export.all-services') }}" method="GET">
                ...
                <button id="btnSubmit" type="submit" class="btn-export">Exportar <span id="icon-export" class="material-icons">file_download</span></button>
    ...
    <div class="card mt-4">
        <div class="chart-container">
            <h4><strong>SERVICIOS</strong> en <strong>TODOS LOS CENTROS</strong></h4>
            <div>
    ...
    <h4>Ventas por empleado de <strong>{{ $selectedService->name }}</strong> en <strong>{{ $selectedCentre->name ?? 'TODOS LOS CENTROS' }}</strong></h4>
    <table class="table mt-4">
        <thead>
            <tr class="row-service" style="background-color: var(--red-icot);color:white;">
                <th>Empleado</th>
