 ...
 <div class="row justify-content-end">
  ...
  <div class="col-xl-6 col-lg-12 column-center">
    <div id="containerImg" class="text-center">
            ...
            <div>
                @php
                    $imagePath = isset($service) && $service->image ? asset('storage/' . $service->image) : asset('storage/img/default.png');
                @endphp
                <img id="serviceImg" src="{{ $imagePath }}" width="600" height="342">
    ...
    <div class="row col-lg-6">
        <div class="col-6">
            ...
            <select name="centre_id" id="centre_id" class="selectpicker" data-style="btn btn-red-icot btn-round" title="Seleccione un Centro" tabindex="-98">
            ...
            <select class="selectpicker text-center" name="category" id="category" class="text-center" data-size="6" data-style="btn btn-red-icot btn-round" title="* Seleccione Categoría de servicio" tabindex="-98">
                @if(isset($categories))
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}" @if (isset($service) && isset($categories) && $category->id == $service->category_id) selected="selected" @endif>{{ $category->name }}</option>
                    @endforeach
                @endif
            </select>
        </div>
    ...
    <div style="margin-right: 60px;">
        <div class="row mr-4 text-right">
            <div class="col-lg-12 ml-4">
                <button id="btnSubmitSave" type="submit" class="btn-save"><span class="material-icons">save</span></button>
                <button id="btnSubmitLoadSave" type="submit" class="btn-save" style="display: none"><span class="btn-save" role="status" aria-hidden="true"></span></button>
                <button id="btnBack" href="/config" class="btn-return"><span class="material-icons">arrow_back</span></button>
            </div>
        </div>
